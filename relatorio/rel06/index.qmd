...
Formalmente, a Distribuição F descreve o comportamento da razão entre duas variáveis aleatórias independentes, cada uma seguindo uma distribuição Qui-quadrado ($\chi^2$), divididas pelos seus respectivos graus de liberdade ($d_1$ e $d_2$):
...
A distribuição qui-quadrado responde a uma pergunta simples: o que acontece quando você pega várias medidas com erro aleatório, eleva cada uma ao quadrado e soma tudo? O resultado dessa soma não segue uma distribuição normal — ele segue uma distribuição qui-quadrado.
